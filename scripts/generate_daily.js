...
function buildPrompt(word){
  ...
  const acts = ['run','jump','sleep','eat','drink']
  const isAction = acts.indexOf(word) >= 0
  const core = isAction
...
const appendSummary = async (line) => {
  try { if (summaryPath) await fs.promises.appendFile(summaryPath, line + '\n', 'utf-8') } catch(e){}
}
function logSavedImage(o) {
  const msg = '[img-ok] ' + o.pathRel + ' <- ' + o.source + ' (' + o.bytes + ' bytes) ' + (o.note || '')
  console.log(msg); appendSummary('- ✅ ' + o.word + ': ' + o.pathRel + '  `' + o.source + '` (' + o.bytes + ' bytes) ' + (o.note || ''))
}
function logPlaceholder(o) {
  const msg = '[img-fallback] ' + o.pathRel + ' (SVG placeholder)'
  console.log(msg); appendSummary('- ⚠️ ' + o.word + ': ' + o.pathRel + ' _(placeholder)_')
}
...
async function genWanx(word){
  ...
  for (let i=0;i<maxRetry;i++){
    try{
      ...
      if (resp.ok){
        let data; try{ data = JSON.parse(text) }catch(e){ console.log('wanx json parse err:', e && e.message); return null }
        const r = data && (data.output && (data.output.results && data.output.results[0] || data.output.choices && data.output.choices[0])) || {}
    ...
    }catch(e){ console.log('wanx err:', e && e.message); await sleep(1000*Math.pow(2,i)) }
  }
  return null
}
...
async function genDoubao(word){
  ...
  const bodyOpen  = { model: DOUBAO_MODEL, prompt: prompt, size: DOUBAO_SIZE, n: 1 }
  ...
  for (const ep of endpoints) {
    ...
    try {
      console.log('[doubao-call]', { url: url, flavor: ep.flavor, model: DOUBAO_MODEL, size: DOUBAO_SIZE, auth: headers['X-API-Key'] ? 'X-API-Key' : 'Bearer' })
      ...
      let data; try { data = JSON.parse(text) } catch(e) {
        console.log('doubao json parse err:', e && e.message)
        continue
      }

      const p = dateImagePath(word)
      const urls = (data && (data.image_urls || (data.data && data.data.image_urls))) || null
      const b64s = (data && (data.binary_data_base64 || (data.data && data.data.binary_data_base64))) || null
      if (urls && urls[0]) {
      ...
      const imgs = (data && data.data && data.data.result && data.data.result.images) || (data && data.result && data.result.images) || data.images
      ...
      if (Array.isArray(dataArr) && dataArr[0] && dataArr[0].b64_json) {
      ...
      if (Array.isArray(dataArr) && dataArr[0] && dataArr[0].url) {
      ...
      console.log('[doubao-err]', e && e.message ? e.message : e)
      await sleep(1000)
    }
  }
...
async function main(){
  ...
  for (const pair of sampled){
    ...
    words.push({
      text: w,
      phonics: '',
      audio: '/media/tts/' + w + '.mp3',
      image: img,
      hint_cn: '指着' + w + '示意；若没有实体，用图片代替'
    })
  }

  const pack = {
    date: ds,
    child_profile: { name: CHILD, interests: [] },
    theme: theme.name,
    words,
    sentences: [
    ...
    story: {
      title:
        (words[0].text[0].toUpperCase() + words[0].text.slice(1)) +
        ' & ' +
        (words[1].text[0].toUpperCase() + words[1].text.slice(1)),
      script_lines:[
    ...
    games:[
      {
        ...
        title:'小小配对官',
        items: words.map(function(w){ return {word: w.text, image: w.image} }),
        correct_feedback:['Yay!','Great job!','High five!'],
        wrong_feedback:['Try again!','Almost!']
      },
      { type:'find-it', title:'找一找颜色', prompt:'Find something red at home!', success_feedback:'You found it! Awesome!' }
    ],
    parent_cards:[
      {
        cn: '指着' + words[0].text + '说：This is a ' + words[0].text + '.',
        en: 'This is a ' + words[0].text + '.',
        tip:'慢速清晰，指物'
      },
      {
        cn: '问：Do you like ' + words[1].text + '?',
        en: 'Do you like ' + words[1].text + '?',
        tip:'引导回答 Yes / No'
      },
