...
function buildPrompt(word){
  return `Cute, kid-friendly illustration of "${word}" on a clean white background, simple bold shapes, bright flat colors, soft round edges, for a 3-year-old learning English; no text; centered; high contrast; minimal shadows`
}
function svg(word){
...
function dateImagePath(word){
  ...
  return { abs: path.join(dir, `${word}.jpg`), rel: `/images/${ds}/${word}.jpg`,
...
async function genWanx(word){
  ...
  try{
    const resp = await fetch('https://dashscope.aliyuncs.com/api/v1/services/aigc/image-generation/generation',{
      ...
      body: JSON.stringify({ model: MODEL, input:{ prompt: buildPrompt(word) }, parameters:{ size: SIZE } })
    })
    if(!resp.ok){ console.log('wanx http', resp.status); return null }
    const data = await resp.json()
    const b64 = data?.output?.results?.[0]?.image_base64 || data?.output?.choices?.[0]?.image_base64
    const url = data?.output?.results?.[0]?.url || data?.output?.choices?.[0]?.url
    const p = dateImagePath(word)
    if (b64){
      fs.writeFileSync(p.abs, Buffer.from(b64, 'base64')); return p.rel
    } else if (url){
      const r2 = await fetch(url); const buf = Buffer.from(await r2.arrayBuffer())
      fs.writeFileSync(p.abs, buf); return p.rel
    }
    return null
  }catch(e){ console.log('wanx err', e); return null }
}
...
async function main(){
  ...
  for (const [w] of sampled){
    ...
    if(!img){
      ...
      fs.writeFileSync(p.absSvg, svg(w)); img = p.relSvg
    }
