...
const SIZE      = process.env.WANX_IMG_SIZE || '512*512'         // 默认用 512*512
const TZ        = process.env.TZ || 'Asia/Shanghai'
const CHILD     = process.env.CHILD_NAME || 'Kid'
const WORKSPACE = process.env.WANX_WORKSPACE || ''               // 可选：业务空间ID
...
async function genWanx(word){
  ...
  const payload = {
    ...
    parameters: { size: SIZE, n: 1 }  // n=1 更省
  }
  ...
  for (let i=0;i<maxRetry;i++){
    try{
      ...
      const text = await resp.text()      // 先拿 text，便于打日志
      if (resp.ok){
        let data
        try { data = JSON.parse(text) } catch(e){ console.log('wanx json parse err:', e?.message); return null }
        const p = dateImagePath(word)
        const r = data?.output?.results?.[0] || data?.output?.choices?.[0] || {}
        if (r.image_base64){
          fs.writeFileSync(p.abs, Buffer.from(r.image_base64, 'base64')); return p.rel
        }
        if (r.url){
          const r2 = await fetch(r.url); const buf = Buffer.from(await r2.arrayBuffer())
          fs.writeFileSync(p.abs, buf); return p.rel
        }
        console.log('wanx ok but empty result for', word, JSON.stringify(data).slice(0,300))
        return null
      }else{
        console.log(`wanx http ${resp.status} for "${word}" body:`, text.slice(0,1000))
